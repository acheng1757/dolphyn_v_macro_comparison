...
if __name__ == "__main__":
    for label in scenario_names:
        xlsx_path = os.path.join(SCRIPT_DIR, f"LCOE_SC_ESC_{label}.xlsx")
        ...
        # lc_summary's cells are formulas referencing lc_detailed; openpyxl never
        # evaluates formulas, so drive real Excel to recalculate before reading it.
        app = xw.App(visible=False)
        try:
            xlwb = app.books.open(xlsx_path)
            app.calculate()
            xlwb.save()

            summary_values = xlwb.sheets["lc_summary"].used_range.value
            xlwb.close()
        finally:
            app.quit()

        csv_path = os.path.join(SCRIPT_DIR, f"LCOE_SC_ESC_{label}.csv")
