...
ASSET_FILES = [
    ("synthetic_ethylene.json", "synthetic_ethylene.csv", SYNTHETIC_ETHYLENE_FIELD_MAP),
]


if __name__ == "__main__":
    # Clear outputs from any previous run before regenerating — case folders
    # created below, plus the downstream LCOE_SYNTHETIC_<case>.csv/.xlsx files
    ...
    for pattern in ("LCOE_SYNTHETIC_*.csv", "LCOE_SYNTHETIC_*.xlsx"):
        for path in glob.glob(os.path.join(SCRIPT_DIR, pattern)):
            if os.path.basename(path) == "LCOE_SYNTHETIC_TEMPLATE.xlsx":
                continue
            os.remove(path)
