...
if __name__ == "__main__":
    ...
    # created below, plus the downstream LCOE_BIO_ETHYLENE_<case>.csv/.xlsx
    ...
    for pattern in ("LCOE_BIO_ETHYLENE_*.csv", "LCOE_BIO_ETHYLENE_*.xlsx"):
        for path in glob.glob(os.path.join(SCRIPT_DIR, pattern)):
            if os.path.basename(path) == "LCOE_BIO_ETHYLENE_TEMPLATE.xlsx":
                continue
            os.remove(path)
